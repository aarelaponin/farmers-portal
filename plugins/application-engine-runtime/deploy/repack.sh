#!/usr/bin/env bash
# Sandbox-friendly build for application-engine-runtime — same pattern as
# form-quality-runtime and identity-resolver-runtime.
set -euo pipefail
...
OUT_JAR="$ROOT/application-engine-runtime-8.1-SNAPSHOT.jar"
WORK="${WORK:-/tmp/ae-build}"

BUILD_FILE="$ROOT/src/main/java/global/govstack/application/Build.java"
...
Bundle-Name: Application Engine Runtime
Bundle-SymbolicName: application-engine-runtime
Bundle-Version: 8.1.0.SNAPSHOT
Bundle-Activator: global.govstack.application.Activator
...
unzip -p "$OUT_JAR" global/govstack/application/Build.class | strings | grep -E '^build-|^[0-9]{4}-' | head -5
echo
