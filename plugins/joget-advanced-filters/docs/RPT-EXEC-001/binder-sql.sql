WITH farmer_data AS (
    SELECT
        ...
        COALESCE(NULLIF(fl.c_field21, ''), '—')              AS sub_centre,
        COALESCE(NULLIF(fl.c_village, ''), '—')              AS village,
    ...
    WHERE 1=1
        ...
        AND (NULLIF('#requestParam.d-5814095-fn_sub_centre?sql#', '') IS NULL
             OR fl.c_field21 ILIKE '%' || '#requestParam.d-5814095-fn_sub_centre?sql#' || '%')
        AND (NULLIF('#requestParam.d-5814095-fn_village?sql#', '') IS NULL
             OR fl.c_village ILIKE '%' || '#requestParam.d-5814095-fn_village?sql#' || '%')
    ...
    GROUP BY d.c_name, cp.c_name, fl.c_field21, fl.c_village, z.c_name
)
...
ORDER BY
    CASE
        ...
        ELSE 0
    END,
    district_name, centre_name, sub_centre, village, zone_name
